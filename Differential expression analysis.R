...
library(ggplot2)
library(ROCR)
data <- read.csv("colorectal_cancer.csv")
## Reading cancer data
str(data)
...
p <- ggplot(data=data, aes(x=log2FoldChange, y=-log10(pvalue))) + geom_point() + theme_minimal()
...
data$delabel <- NA
data$delabel[data$diffexpressed != "NO"] <- data$gene_symbol[data$diffexpressed != "NO"]
